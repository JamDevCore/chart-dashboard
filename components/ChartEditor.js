...
const ChartEditor = ({ chartData, field, dimension, measure, title, type, id , setCharts, charts, saveCharts, setSaveCharts , setChartModalIsOpen, setSelectedChart}) => {
  ...
  return (
    <div className="flex flex-col">
        ...
        <div>
            ...
            {numericalFields.map(field => <option>{field}</option>)}
            </select>
            </div>
            <div>
            <label htmlFor="location" className="block text-sm font-medium text-gray-700">
            ...
            >
                ...
                {stringFields.map(field => <option>{field}</option>)}
            </select>
            </div>
            <div>
            <label htmlFor="location" className="block text-sm font-medium text-gray-700">
            ...
            onClick={() => {
                ...
                if(typeof id === 'number') {
                    setCharts(charts.map((chart, index) => {
                        if(index === id) {
                            return { field: selectedField, dimension: selectedDimension, type: selectedType, title: selectedTitle, measure: selectedMeasure }
                        }
                        return chart;
                    }))
                } else {
                    const newChartSet = [{ field: selectedField, dimension: selectedDimension, type: selectedType, title: selectedTitle, measure: selectedMeasure }].concat(charts);
